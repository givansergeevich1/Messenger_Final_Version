    ...
    public class FirebaseDatabaseService : IDatabaseService
    {
        ...
        public async Task<List<Message>> SearchMessagesAsync(string chatId, string searchQuery)
        {
            try
            {
                ...
                var allMessages = await GetChatMessagesAsync(chatId, 1000);
                var query = searchQuery.ToLowerInvariant().Trim();
        ...
        ~FirebaseDatabaseService()
        {
            try
            {
                ...
                foreach (var subscription in _chatSubscriptions.Values)
                {
                    subscription?.Dispose();
                }
            ...
            {
                Console.WriteLine($"Error in FirebaseDatabaseService destructor: {ex.Message}");
            }
        }
    }
}